...
In addition to the cross-section calculations, collider physicists often need simulated events to compare with data. Several main stages are required to simulate the full picture of a hadron collision event down to the level of stable particles before the detector response kicks in. These main stages include: hard scattering, \ac{PS}, hadronization, underlying event, and unstable particle decay.
...
Unstable hadrons produced in the hadronization process will decay sequentially, which is typically the final stage of event generation. These decays are often simulated by phenomenological models, such as the EvtGen~\cite{Lange:2001uf}, which take input from \ac{PDG}~\cite{ParticleDataGroup:2022pth}.