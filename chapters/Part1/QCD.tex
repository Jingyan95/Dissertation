...
As \acp{PDF} only describe nonperturbative effects below the energy scale $\mu_{F}$, they must be extracted by experimentalists from data~\cite{NNPDF:2014otw,NNPDF:2017mvq}. Analogous to the running of the strong coupling constant $\alpha_{S}$, the exact details of \acp{PDF} depend on the energy scale at which it is evaluated. The \acp{PDF} extracted at energy scale $\mu_{F}$ can be related to the \acp{PDF} at another scale energy scale by the so called ``DGLAP'' equation~\cite{Gribov:1971zn,Altarelli:1977zs,Dokshitzer:1977sg}. The solutions to the ``DGLAP'' are referred to as the renormalized \acp{PDF}, which can be applied universally to any processes. 

Descriptions of the short-distance physics is contained in the third factor of Equation~\ref{eq:master}, $\hat{\sigma}_{ab\rightarrow k}$, which is the partonic cross-section for the production of partonic final states $k$ from initial partons $i$ and $j$. The initial partons $i$ and $j$ can be quarks, anti-quarks, or gluons, and all possible initial states to produce $k$ are summed over and wighted by the probabilities given by the \acp{PDF}. The partonic cross-section is given by:
...
The last factor in Equation~\ref{eq:master} are the fragmentation functions~\cite{Field:1976ve} that encode information on how partons produced in hard scatterings are turned into colorless bound states $X$, which may be observed in the detector. More specifically, fraction $z$ of the parton momentum is transferred to the hadron $X$, and the associated probability at energy scale $\mu_{F}$ is represented by $D_{k\rightarrow X}(z,\mu_{F}^2)$. Similar to the situation of the \ac{PDF} determination, the fragmentation falls into the nonperturbative regime of \ac{QCD}, which can not be predicted precisely by theoretical methods. Therefore, the fragmentation functions must be extracted from experiments.  

In addition to the cross-section calculations, collider physicists often need simulated events to compare with data. Several main steps are involved in the simulation of the full picture of a hadron collision event before detector response kicks in. These main steps typically include hard scattering, parton shower, hadronization, underlying event, and unstable particle decays.

Generation of the hard scattering events is usually done by the \ac{ME} event generator, such as \MG~\cite{Alwall:2014hca} and \Pow~\cite{Frixione:2007vw}. Depending on the process in question, these \ac{ME} generators determine the relevant Feynman diagrams and uses \ac{MC} methods to generate final-state particles with properties such as particle types, four-momenta, and spins. Each generated event is weighted by the corresponding transition rate, which is calculated numerically using perturbative theory. The \ac{NLO} expansions are commonly used in these perturbative calculations, which typically means one extra parton is involved in the \ac{ME}.

Beyond the \ac{NLO}, higher order \ac{QCD} effects are modeled by the parton shower, which uses splitting functions to characterize the emissions of soft partons by the inital-state and final state partons. Since the emited partons can further emit partons themselves, this process will include soft partons with lower and lower energy until perturbative theory breaks down, and finally producing the parton showers. 

The hadronization process takes account of the confinement of partons into the formation of the observed final-state hadrons. The event generators rely on string or cluster. model for simulation of this step. 

The underlying event comes from the interactions between those partons in the incoming hadrons that do not directly participate in the hard subprocess. It produces soft hadrons everywhere in the event and contaminate the hard process that was already simulated. 

The last stage of event generation is the sequential decay of those unstable hadrons produced in the hadronization process. One of the commonly used tools to implement parton shower is \PY~\cite{Sjostrand:2014zea}.